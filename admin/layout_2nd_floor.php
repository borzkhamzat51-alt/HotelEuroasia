...
function rl_room_data($room, $activeRes, $rooms = []) {
    return [
        'id'                 => $room['id'],
...
$leftRooms   = ['203','202','201'];
$middleRooms = ['206','205','204'];
$rightRooms  = ['210','209','208','207'];
$columns     = ['left' => [], 'middle' => [], 'right' => []];
foreach ($rooms as $room) {
    $num = $room['room_number'];
    $rd  = rl_room_data($room, rl_active_res($reservationsByRoom[$room['id']] ?? []), $rooms);
...
<title>2nd Floor Layout · Bluebookers</title>
...
<div class="rl-heading">
    ...
    <h1 class="rl-title">2nd Floor Layout</h1>
    <div class="rl-floor-switch">
        <a href="layout_1st_floor.php?branch=<?= urlencode($branch) ?>" class="rl-floor-btn">1st Floor</a>
        <a href="layout_2nd_floor.php?branch=<?= urlencode($branch) ?>" class="rl-floor-btn is-active">2nd Floor</a>
