...
function rl_current_reservation($reservations) {
    ...
    $future = null;
    error_log("[rl_current_reservation] Checking " . count($reservations) . " reservations.");
    foreach ($reservations as $r) {
        if (in_array($r['status'], ['cancelled', 'checked_out'], true)) {
            error_log("[rl_current_reservation] Skipping cancelled/checked_out reservation ID: " . $r['id']);
            continue;
        }
        $checkIn = new DateTime($r['check_in']);
        $checkOut = new DateTime($r['check_out']);
        if ($checkIn <= $today && $checkOut > $today) {
            error_log("[rl_current_reservation] Found active reservation ID: " . $r['id'] . " for today.");
            return $r; // active reservation
        }
        if ($checkIn > $today) {
            if ($future === null || $checkIn < new DateTime($future['check_in'])) {
                $future = $r;
                error_log("[rl_current_reservation] Found future reservation ID: " . $r['id'] . " with check_in: " . $r['check_in']);
            }
        }
    }
    if ($future) {
        error_log("[rl_current_reservation] Returning future reservation ID: " . $future['id']);
    } else {
        error_log("[rl_current_reservation] No future or active reservation found.");
    }
...
foreach ($rooms as $room) {
    ...
    error_log("[layout] Room ID: " . $room['id'] . " (" . $num . ") has " . count($reservations) . " reservations.");
    $currentRes = rl_current_reservation($reservations);
    if ($currentRes) {
        error_log("[layout] Room " . $num . " has current reservation ID: " . $currentRes['id']);
    } else {
        error_log("[layout] Room " . $num . " has NO current reservation.");
    }
    if (in_array($num, $leftRooms, true)) {
        $columns['left'][] = ['room' => $room, 'res' => $currentRes];
    } elseif (in_array($num, $middleRooms, true)) {
        $columns['middle'][] = ['room' => $room, 'res' => $currentRes];
